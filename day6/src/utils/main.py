...
async def user_service(url, id, session):
    ...
    async with session.get(user_service_url) as response:
        if response.status == 200:
            ...
            print(f"Failed to fetch user data for id {id}. Status code: {response.status}")
            return None
...
async def post_service(url, id, session):
    ...
    async with session.get(post_service_url) as response:
        if response.status == 200:
            ...
            print(f"Failed to fetch post data for id {id}. Status code: {response.status}")
            return None
...
async def album_service(url, id, session):
    ...
    async with session.get(album_service_url) as response:
        if response.status == 200:
            ...
            print(f"Failed to fetch album data for id {id}. Status code: {response.status}")
            return None
...
async def photo_service(url, id, session):
    ...
    async with session.get(photo_service_url) as response:
        if response.status == 200:
            ...
            print(f"Failed to fetch photo data for id {id}. Status code: {response.status}")
            return None

async def dashboard(url):
    async with aiohttp.ClientSession() as session:
        #concurently fetch data from all services using asyncio.gather
